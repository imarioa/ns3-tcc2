...
then
	...
	do
		...
		do
			echo -ne "$numSeed \r"
  			./ns3 run "lorawan-network-mClass-sim-att --nSeed=$RANDOM --nDevices=$numSta --nGateways=$gwRing --radius=$rad --gatewayRadius=$gwRad --simulationTime=$simTime --appAtkPeriod=60 --appPeriod=$interval --file1=$file1 --file2=$file2 --print=$pEDs --trial=$trial"  > ./TestResult/test$trial/traffic-$interval/pcap-sta-$numSta/record-$numSta.txt
			#./ns3 run "lorawan-network-mClass-sim --nSeed=$RANDOM --nDevices=$numSta --nGateways=$gwRing --radius=$rad --gatewayRadius=$gwRad --simulationTime=$simTime --appPeriod=$interval --file1=$file1 --file2=$file2 --print=$pEDs --trial=$trial"  > ./TestResult/test$trial/traffic-$interval/pcap-sta-$numSta/record-$numSta.txt 2>&1
		done
	echo ""
	done
elif [ $1 -eq 1 ]
then
	...
	do
		...
		do
			echo -ne "$numSeed \r"
 			./ns3 run "lorawan-network-sim-att --nSeed=$RANDOM --nDevices=$numSta --nAtkDevices=6 --nGateways=$gwRing --radius=$rad --gatewayRadius=$gwRad --simulationTime=$simTime --appPeriod=$interval --appAtkPeriod=60 --file1=$file1 --file2=$file2 --print=$pEDs --trial=$trial" > ./TestResult/test$trial/traffic-$interval/pcap-sta-$numSta/record-$numSta.txt 2>&1
		done
	done
elif [ $1 -eq 2 ]
then
